/**
 * EngineTab — AI Automation Control Panel
 * Extracted from app.engine.jsx for the unified Command Center.
 */

import { useState, useCallback, useRef, useEffect } from "react";
import { useAppBridge } from "@shopify/app-bridge-react";
import { StatCard, ConfigSlider, ConfigToggle, DetailTable, LifecycleBadge, ProcessLog } from "../engine-ui";
import { MonitoringDashboard } from "../engine/MonitoringDashboard";
import { SchedulingConfig } from "../engine/SchedulingConfig";
import { AlertsConfig } from "../engine/AlertsConfig";
import { fmt$ } from "../../lib/format";

const NICHES = [
  { value: "gym", label: "Gym & tactical", icon: "\u{1F3CB}️" },
  { value: "anxiety", label: "Wellness", icon: "\u{1F9E0}" },
  { value: "home", label: "Home & kitchen", icon: "\u{1F3E0}" },
  { value: "pet", label: "Pet accessories", icon: "\u{1F43E}" },
  { value: "tech", label: "Tech & gadgets", icon: "\u{1F4F1}" },
  { value: "beauty", label: "Beauty & skincare", icon: "\u{1F484}" },
];
...
export function EngineTab({ savedConfig }) {
  ...
  return (
    <>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
        <s-text variant="headingMd">Product Lifecycle Engine</s-text>
        <div>
          {running ? (
            <s-button
              variant="destructive"
                        ...
                        >
                          ...
                          <div style={{ flex: 1, minWidth: 0 }}>
                            <div style={{ fontSize: 12, fontWeight: 600, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
                              {p.name}
                              {p.imported && <span style={{ color: "var(--p-color-text-success)", fontSize: 10, marginLeft: 6 }}>OK listed</span>}
                            </div>
                            ...
                            {showAdvanced && (
                              <div style={{ fontSize: 10, color: "var(--p-color-text-secondary)", marginTop: 3, display: "flex", gap: 10 }}>
                                ...
                                {p.warns?.length > 0 && (
                                  <span style={{ color: "var(--p-color-text-critical)" }}>Warning: {p.warns.join(", ")}</span>
                                )}
                              </div>
                            )}
                          </div>
